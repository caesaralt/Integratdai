...
# --- JSON file helpers (GPT additions) ---
...
# --- GPT: latest editor redirect so the menu link works ---
@app.route('/editor/latest')
def editor_latest():
    try:
        learning_index = load_learning_index()
        examples = learning_index.get('examples', []) if isinstance(learning_index, dict) else []
        if not examples:
            return redirect(url_for('quotes_page'))
        latest = max(examples, key=lambda e: e.get('timestamp', ''))
        pid = latest.get('timestamp') or latest.get('id') or latest.get('project_id')
